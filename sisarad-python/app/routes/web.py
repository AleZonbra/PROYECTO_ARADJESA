...
from app.auth import contexto_usuario, requiere_admin_sistema, requiere_modulo, require_user
from app.roles import ADMINISTRADOR_SISTEMA, ENCARGADO_NEGOCIO, etiqueta_rol, normalizar_rol
...
def _actualizar_sesion_usuario(request: Request, user: Usuario):
    request.session["user"] = {
        ...
        "role": user.role,
        "role_label": etiqueta_rol(user.role),
    }
...
def login_submit(
    ...
    if rol_normalizado and rol_normalizado != user.role:
...
def productos_list(request: Request, db: Session = Depends(get_db)):
    ...
    items = _filter_items(db.query(Producto).order_by(Producto.id).all(), search, ["producto", "serial_lote"])
    paged = paginate(items, page, ITEMS_PER_PAGE)
    edit_item = _get_by_id(db, Producto, request.query_params.get("edit"))
    view_item = _get_by_id(db, Producto, request.query_params.get("ver"))
    return request.app.state.templates.TemplateResponse(
        "productos.html",
        _ctx(
            request,
...
@protected_router.post("/productos/{item_id}/actualizar", dependencies=[Depends(requiere_modulo("productos"))])
...
@protected_router.post("/productos/{item_id}/eliminar", dependencies=[Depends(requiere_modulo("productos"))])
...
@protected_router.post("/vendedores/{item_id}/actualizar", dependencies=[Depends(requiere_modulo("vendedores"))])
...
@protected_router.post("/vendedores/{item_id}/eliminar", dependencies=[Depends(requiere_modulo("vendedores"))])
def vendedores_eliminar(item_id: int, db: Session = Depends(get_db)):
    item = db.get(Vendedor, item_id)
    if item:
        db.delete(item)
        db.commit()
    return _redirect("/vendedores", msg="Vendedor+eliminado")
...
@protected_router.post("/proveedores/{item_id}/actualizar", dependencies=[Depends(requiere_modulo("proveedores"))])
...
@protected_router.post("/proveedores/{item_id}/eliminar", dependencies=[Depends(requiere_modulo("proveedores"))])
def proveedores_eliminar(item_id: int, db: Session = Depends(get_db)):
    item = db.get(Proveedor, item_id)
    if item:
        db.delete(item)
        db.commit()
    return _redirect("/proveedores", msg="Proveedor+eliminado")
...
@protected_router.post("/clientes/{item_id}/actualizar", dependencies=[Depends(requiere_modulo("clientes"))])
...
@protected_router.post("/clientes/{item_id}/eliminar", dependencies=[Depends(requiere_modulo("clientes"))])
...
@protected_router.post("/usuarios/{item_id}/actualizar", dependencies=[Depends(requiere_modulo("usuarios"))])
...
@protected_router.post("/usuarios/{item_id}/eliminar", dependencies=[Depends(requiere_modulo("usuarios"))])
...
@protected_router.post("/despachos/{item_id}/actualizar", dependencies=[Depends(requiere_modulo("despachos"))])
...
@protected_router.post("/despachos/{item_id}/eliminar", dependencies=[Depends(requiere_modulo("despachos"))])
